...
public class SendMail : IHttpHandler
{
        ...
        if (success)
        {
            ...
            if ((!string.IsNullOrEmpty(fromEmail)) && (!string.IsNullOrEmpty(comment)))
            {
                ...
                using (System.Net.Mail.MailMessage message = new System.Net.Mail.MailMessage())
                {
                    ...
                    message.Subject = "CA @50 Million Confirmation";
                    message.From = new System.Net.Mail.MailAddress("no-reply@state.ca.gov");
    ...
    {
        bool success = false;
        string postDataString = string.Format("&secret=6Le7EBwTAAAAANTPZKCCM9ZgCLYOTKxPSa9oMKXi&response={0}&remoteip=", gRecaptchaResponse);
